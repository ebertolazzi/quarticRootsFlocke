...
namespace PolynomialRoots
{
  ...
  static void QuadraticSyntheticDivision(
    integer const   NN,
    real_type const u,
    real_type const v,
    real_type const p[],
    ...
    real_type       qk[] )
  {
    ...
    if ( std::abs( c ) <= epsilon100 * std::abs( K[N - 1] ) && std::abs( d ) <= epsilon100 * std::abs( K[N - 2] ) )
      return dumFlag;
    ...
    real_type const qp[] )
  {
    ...
    else
    {
      if ( real_type const temp{ tFlag == 1 ? b : a }; std::abs( a1 ) > epsilon10 * std::abs( temp ) )
      {
        ...
        K[0] = qp[0];
        K[1] = qp[1] - ( a7 * qp[0] );
        for ( integer i = 2; i < N; ++i ) K[i] = qp[i] - ( a7 * qp[i - 1] ) + a3 * qk[i - 2];
      }
      else
      {
        ...
        K[0] = 0;
        K[1] = -a7 * qp[0];
        for ( integer i = 2; i < N; ++i ) K[i] = a3 * qk[i - 2] - a7 * qp[i - 1];
      }
    }
  }
    ...
    real_type       qk[] )
  {
    ...
    for ( integer j = 0;; )
    {
      ...
      qp[0]        = pv;
      for ( integer i = 1; i < NN; ++i ) qp[i] = pv = pv * s + p[i];
      ...
      for ( integer i = 1; i < N; ++i ) qk[i] = kv = kv * s + K[i];
  ...
  */
  ...
  static real_type evalPoly( real_type const x, real_type const p[], integer const Degree )
  {
    real_type ff = p[0];
  ...
  static void evalPoly( real_type const x, real_type const p[], integer const Degree, real_type & f, real_type & df )
  {
    df = f = p[0];
  ...
  */
  static void scalePoly( real_type p[], integer const N )
  {
    /*
  ...
  static real_type lowerBoundZeroPoly( real_type p[], integer const N )
  {
    ...
    if ( pt[static_cast<std::size_t>( N - 1 )] != 0 )
    {  // If Newton step at the origin is better, use it
      if ( real_type const xm{ -pt[static_cast<std::size_t>( N )] / pt[static_cast<std::size_t>( N - 1 )] }; xm < x )
        x = xm;
    }
    // Chop the interval(0, x) until f <= 0
    real_type xm{ x };
    while ( evalPoly( xm, pt.data(), N ) > 0 )
    {
      x  = xm;
      xm = 0.1 * x;
    }
  ...
  static void roots3( real_type const p[4], integer const Degree, real_type zeror[], real_type zeroi[] )
  {
    if ( Degree == 1 )
    {
      zeror[0] = -( p[1] / p[0] );
  ...
  int roots( real_type const * op, integer const Degree, real_type * zeror, real_type * zeroi )
  {
    ...
    for ( integer j = 0; op[N] == 0; ++j, --N ) zeror[j] = zeroi[j] = 0.0;
    ...
    while ( N > 0 )
    {
      ...
      if ( N < 4 )
      {
        roots3( p, N, zeror + Degree - N, zeroi + Degree - N );
        break;
      }
